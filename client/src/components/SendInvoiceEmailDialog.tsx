...
function SendInvoiceEmailDialog({ invoiceId, invoiceType, clientEmail, contactName, projectTitle, onClose, onSent }: Props) {
  ...
  useEffect(() => {
    fetch(`${API_URL}/api/invoices/${invoiceId}/emails`)
  ...
  async function send(event: React.FormEvent) {
    ...
    setBusy(true)
    try {
      const response = await fetch(`${API_URL}/api/invoices/${invoiceId}/send-email`, {
        method: 'POST',
  ...
  return (
    <div className="modal-overlay" onMouseDown={(event) => event.target === event.currentTarget && onClose()}>
      <div className="modal modal-wide" role="dialog" aria-modal="true" aria-labelledby="send-invoice-email-title">
        <div className="modal-header">
          <h2 id="send-invoice-email-title">Send {invoiceType === 'client_funds' ? 'Client Funds Document' : 'Invoice'} by Email</h2>
